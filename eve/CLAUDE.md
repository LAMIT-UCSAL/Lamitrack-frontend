...
- **Removido do exemplo original:** Framer Motion/`motion`, `react-use-measure` — todo o cálculo de largura/posição usa `track.scrollWidth` medido direto do DOM. O `reverse`/direção alternativa do exemplo original não foi replicado (deixaria parceiros e estatísticas deslizando em sentidos opostos, inconsistente).
- **Moldura:** `[mostrarDivisores]` controla os divisores horizontais full-bleed (100vw) no topo/base — ligado na faixa de parceiros (seção isolada), desligado nas estatísticas (já dentro de uma seção com suas próprias bordas, evitando duplicar linhas). Sem a caixa central de 768px do exemplo React original — largura total da tela, removida a pedido do usuário.
...
- **Logos dos parceiros:** mantidos os parceiros reais já usados (GDG Salvador, NXOS, SCUPP, Stem, Rádio, Red Bull, GDG Lauro, Rocketseat) — nada de placeholder/wordmark genérico do exemplo original. Sem o filtro `dark:invert` do exemplo (o projeto não tem modo escuro).
