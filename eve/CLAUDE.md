...
├── shared/
│   ├── components/    # navbar, evento-card, carrossel-eventos, globo-eve, marquee
...
- **`prefers-reduced-motion`:** com a preferência ativa, a rotação automática para (velocidade zero), mas o globo continua arrastável manualmente — mesma filosofia de "pausa, não remove" aplicada ao componente `marquee` e ao carrossel de eventos (ver Elevation/Motion no `DESIGN.md`).

## Marquee genérico (`shared/components/marquee`)

Componente standalone reutilizável, usado hoje em **dois lugares da home**: a faixa de logos de parceiros e a tira de estatísticas do hero (`stats`). Nasceu como um componente só de logos ("logo-cloud", adaptado de um exemplo React/shadcn/Tailwind — "logo-cloud-4", `InfiniteSlider` + `ProgressiveBlur` com Framer Motion, do 21st.dev) e foi generalizado depois que o usuário pediu o mesmo tratamento visual/comportamental para as estatísticas — em vez de duplicar a lógica, o motor (RAF + blur + desaceleração no hover) virou um componente único parametrizado, e o conteúdo de cada item é injetado via `@Input() itemTemplate: TemplateRef<any>` (`*ngTemplateOutlet`), com `@Input() items` fornecendo os dados. Cada tela chamadora (`home.component.html`) define seu próprio `<ng-template #xTpl let-item>` com o markup do item (imagem de logo, ou par número+rótulo) — o `MarqueeComponent` não sabe nada sobre logos ou estatísticas, só duplica a lista e renderiza o template passado.

Detalhes da adaptação/generalização:
- **Movimento:** em vez de uma `@keyframes` CSS de duração fixa, o deslocamento roda em `requestAnimationFrame` (fora da zone, `NgZone.runOutsideAngular`) com velocidade interpolada suavemente (lerp) entre o estado normal e o de hover — isso reproduz o comportamento do `speedOnHover` do Framer Motion sem precisar da lib. Ao passar o mouse a faixa **desacelera, nunca para** (regra herdada de uma iteração passada: pausar no hover foi explicitamente proibido pelo usuário).
- **Blur progressivo nas bordas:** em vez do simples `mask-image` de fade usado na primeira versão da marquee de parceiros, um efeito de blur em camadas (`estiloCamada()` gera 6 `<div>`s absolutos por lado, cada um com uma banda de `mask-image` + `backdrop-filter: blur()` progressivamente mais forte) — mesma técnica do `ProgressiveBlur` original, calculada em TypeScript puro em vez de componente React separado. `[blurCompacto]` encolhe a banda (32px→96px) pra itens mais estreitos como os pares de estatística.
- **Removido do exemplo original:** Framer Motion/`motion`, `react-use-measure` — o cálculo de largura/posição mede o DOM direto via `ViewChildren('itemEl')`.
- **Moldura:** `[mostrarDivisores]` controla os divisores horizontais full-bleed (100vw) no topo/base — ligado na faixa de parceiros (seção isolada), desligado nas estatísticas (já dentro de uma seção com suas próprias bordas, evitando duplicar linhas). Sem a caixa central de 768px do exemplo React original — largura total da tela, removida a pedido do usuário. O `reverse`/direção alternativa do exemplo original não foi replicado (deixaria parceiros e estatísticas deslizando em sentidos opostos, inconsistente).
- **`prefers-reduced-motion`:** com a preferência ativa, a faixa fica estática (velocidade zero) mas todo o conteúdo continua visível e legível, sem esconder nada atrás da animação.
- **Logos dos parceiros:** mantidos os parceiros reais já usados (GDG Salvador, NXOS, SCUPP, Stem, Rádio, Red Bull, GDG Lauro, Rocketseat), em 70px de altura (`.parceiro-logo-img` em `home.component.scss`) — nada de placeholder/wordmark genérico do exemplo original. Sem o filtro `dark:invert` do exemplo (o projeto não tem modo escuro).
- **Bug corrigido — corte visível no loop:** a primeira versão calculava o ponto de reset do loop como `track.scrollWidth / 2`. Isso erra por exatamente meio `gap` sempre que os itens têm espaçamento via flex `gap` (em vez de `margin-right`) numa lista duplicada com número ímpar de gaps no total — o mesmo tipo de bug de "meio gap" já visto antes neste projeto na técnica CSS `translateX(-50%)`. A correção mede a distância real no DOM entre o primeiro item e seu duplicado (`ViewChildren('itemEl')`, offset do item `N` menos o do item `0`) em vez de assumir metade da largura total — e reage a `ResizeObserver` no `track` pra remedir se logos/fontes mudarem de tamanho depois de carregar.
